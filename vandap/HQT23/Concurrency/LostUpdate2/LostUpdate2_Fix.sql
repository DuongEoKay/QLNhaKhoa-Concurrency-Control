...
CREATE OR ALTER PROC SP_UPDATE_HOSOBENHNHAN_LU_FIX
	@IDHOSO VARCHAR(10),
    @SONGAYDOI INT
AS BEGIN TRAN
    IF @SONGAYDOI = 0
	...
	DECLARE @IDNHASI VARCHAR(5)
	SELECT @IDNHASI = NhaSi FROM HoSoBenhNhan WHERE IDHoSo = @IDHOSO

	DECLARE @NGAYCU DATE
	SELECT @NGAYCU = ThoiGianKham FROM HoSoBenhNhan WITH(XLOCK) WHERE IDHoSo = @IDHOSO
